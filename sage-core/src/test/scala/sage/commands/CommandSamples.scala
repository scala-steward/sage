...
object CommandSamples {
  ...
  val all: Vector[Sample] = Vector(
    ...
    Sample(Json.jsonSet("k", JsonPath.root, "v"), Vector("JSON.SET", "k", "$", "v")),
    Sample(Json.jsonSet("k", JsonPath("$.a"), "v", JsonSetCondition.IfNotExists), Vector("JSON.SET", "k", "$.a", "v", "NX")),
    Sample(Json.jsonSet("k", JsonPath("$.a"), "v", JsonSetCondition.IfExists), Vector("JSON.SET", "k", "$.a", "v", "XX")),
    Sample(Json.jsonGet[String, String]("k"), Vector("JSON.GET", "k")),
    Sample(Json.jsonGet[String, String]("k", JsonPath("$.a"), JsonPath("$.b")), Vector("JSON.GET", "k", "$.a", "$.b")),
    Sample(Json.jsonMGet[String, String]()("k1", "k2"), Vector("JSON.MGET", "k1", "k2", "$")),
    Sample(Json.jsonMGet[String, String](JsonPath("$.a"))("k1"), Vector("JSON.MGET", "k1", "$.a")),
    Sample(Json.jsonMSet(("k", JsonPath.root, "v")), Vector("JSON.MSET", "k", "$", "v")),
    Sample(Json.jsonMSet(("k1", JsonPath.root, "v1"), ("k2", JsonPath("$.a"), "v2")), Vector("JSON.MSET", "k1", "$", "v1", "k2", "$.a", "v2")),
    Sample(Json.jsonMerge("k", JsonPath.root, "v"), Vector("JSON.MERGE", "k", "$", "v")),
    Sample(Json.jsonDel("k"), Vector("JSON.DEL", "k", "$")),
    Sample(Json.jsonDel("k", JsonPath("$.a")), Vector("JSON.DEL", "k", "$.a")),
    Sample(Json.jsonClear("k"), Vector("JSON.CLEAR", "k", "$")),
    Sample(Json.jsonType("k"), Vector("JSON.TYPE", "k", "$")),
    Sample(Json.jsonToggle("k", JsonPath("$.a")), Vector("JSON.TOGGLE", "k", "$.a")),
    Sample(Json.jsonStrAppend("k", JsonPath.root, "v"), Vector("JSON.STRAPPEND", "k", "$", "v")),
    Sample(Json.jsonStrLen("k"), Vector("JSON.STRLEN", "k", "$")),
    Sample(Json.jsonNumIncrBy("k", JsonPath.root, 5.0), Vector("JSON.NUMINCRBY", "k", "$", "5.0")),
    Sample(Json.jsonNumMultBy("k", JsonPath.root, 2.0), Vector("JSON.NUMMULTBY", "k", "$", "2.0")),
    Sample(Json.jsonArrAppend("k", JsonPath.root, "1", "2"), Vector("JSON.ARRAPPEND", "k", "$", "1", "2")),
    Sample(Json.jsonArrIndex("k", JsonPath.root, "1"), Vector("JSON.ARRINDEX", "k", "$", "1", "0", "0")),
    Sample(Json.jsonArrIndex("k", JsonPath("$.a"), "1", 2L, 5L), Vector("JSON.ARRINDEX", "k", "$.a", "1", "2", "5")),
    Sample(Json.jsonArrInsert("k", JsonPath.root, 0L, "1", "2"), Vector("JSON.ARRINSERT", "k", "$", "0", "1", "2")),
    Sample(Json.jsonArrLen("k"), Vector("JSON.ARRLEN", "k", "$")),
    Sample(Json.jsonArrPop[String, String]("k"), Vector("JSON.ARRPOP", "k", "$", "-1")),
    Sample(Json.jsonArrPop[String, String]("k", JsonPath("$.a"), 0L), Vector("JSON.ARRPOP", "k", "$.a", "0")),
    Sample(Json.jsonArrTrim("k", JsonPath.root, 0L, 1L), Vector("JSON.ARRTRIM", "k", "$", "0", "1")),
    Sample(Json.jsonObjKeys("k"), Vector("JSON.OBJKEYS", "k", "$")),
    Sample(Json.jsonObjLen("k"), Vector("JSON.OBJLEN", "k", "$")),
    Sample(Json.jsonDebugMemory("k"), Vector("JSON.DEBUG", "MEMORY", "k", "$")),
    Sample(Json.jsonResp("k"), Vector("JSON.RESP", "k", "$")),
